...
<div class="box_tool_min padding_top2 padding_bottom2 padding_right5" style="margin-bottom: 0px; ">
        <a href="news_add.do" style="display: block; width: 77px; height: 28px; background-color: #3ba2c0;
...
<div id="scrollContent" style="overflow-x:hidden;">
	...
	private String name; -->
	       <tr>
                ...
                <td align="center" title="">${data.title}</td>
                <td align="center">${data.createTime}</td>
                
                <td align="center" style="margin-right: 3px">
                <c:if test="${role == 1 }">
                   <a href="news_update.do?id=${data.id}"> <span style="color: #f9a002" >修改</span></a>
                         <a href="javascript:" onclick="del('${data.id}')"> <span style="color: red">删除</span></a>
                </c:if>
              
                         <a href="javascript:" onclick="view('${data.id}')"> <span style="color: red">查看</span></a>
                </td>
            </tr>
             </c:forEach>
...
<script>
  ...
  /*   function onAdd() {
    ...
    //查看详情
    function view(workOrderId) {
        top.Dialog.open({
            Module: 'dxgd',
            URL: "news_view.do?id="+workOrderId,
            Title: "详情", Width: 600, Height: 400
        });
    }
   ...
   function postData() {
    ...
    function del(id){
    	top.Dialog.confirm("是否确认删除",function(){
    		location.href=ctx+"/news_delete.do?id="+id;
    	},function(){
